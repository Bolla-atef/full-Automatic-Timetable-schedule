  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/Schedule/AllUniqueStudent`,
        {
          headers: {
...
const handleDeleteStudent = useCallback(async () => {
  ...
  try {
    ...
    await axios.delete(
      `${import.meta.env.VITE_API_URL}/api/Schedule/DeleteStudent/${selectedStudent.studentId}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
...
const studentOptions = useMemo(() => {
  ...
  return filteredStudents
    .filter(group => group[0]) // نتأكد إن فيه بيانات
    ...
    <div className="background-main-pages">
      ...
      <div className="max-w-screen-xl mx-auto rounded-md sm:px-6">
        ...
        <div className="mb-6">
          ...
          >
            {studentOptions.length > 0 ? studentOptions : <Option value="" disabled>No students found</Option>}
          </Select>
        </div>
