...
const ProfessorCoursesManager = ({
  ...
  const { data: courseProfessors = [], isLoading: assignedLoading } =
    useQuery({
      ...
      queryFn: async () => {
        const token = localStorage.getItem("userToken");
        const { data } = await axios.get(
          `${import.meta.env.VITE_API_URL}/api/CourseProfessors`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
  ...
  const assignMutation = useMutation({
    mutationFn: (courseId) =>
      axios.post(
        `${import.meta.env.VITE_API_URL}/api/CourseProfessors/${professorId}/${courseId}`,
        {},
        {
          headers: {
...
const GetProfessors = () => {
  ...
  const mutation = useMutation({
    mutationFn: async (payload) => {
      ...
      const url = isEdit
  ? `${import.meta.env.VITE_API_URL}/api/Professors/${payload.id}`
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id) => {
      const token = localStorage.getItem("userToken");
      await axios.delete(
        `${import.meta.env.VITE_API_URL}/api/Professors/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  const handleSubmit = (e) => {
    ...
    if (!professor.name.trim() || professor.availability.length === 0) {
      toast.error("يرجى إدخال اسم البروفيسور والأيام المتاحة");
      return;
    }
                          ...
                          filteredProfessors.map((professor) => (
                            <Fragment key={professor.id}>
                              <tr key={professor.id} className="hover:bg-black">
                                ...
                                <td className="px-5 py-4 whitespace-nowrap text-sm text-white">
                                  {courseProfessors.filter(cp => cp.idProfessor === professor.id).length}
                                </td>
