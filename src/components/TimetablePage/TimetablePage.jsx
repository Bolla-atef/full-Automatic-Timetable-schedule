...
import { getYearLabel as getDeptYearLabel } from "../config/departmentsConfig";

const WeeklyTimetable = () => {

  // College info for dynamic logos
  const college = getSelectedCollege();
  const collegeId = localStorage.getItem("selectedCollege") || "engineering";

  // Dynamic year label using departments config
  const getYearLabel = (year) => getDeptYearLabel(year, collegeId);
  ...
  logoLeft.src = college.logoLeft;
  logoLeft.style.height = '60px';
  logoLeft.style.width = 'auto';
  logoLeft.crossOrigin = 'anonymous';
  ...
  titleDiv.innerHTML = `
    <div style="font-size:16px;font-weight:bold;color:#2d3748;">Academic Year: ${getYearLabel(selectedYear)}</div>
    <div style="font-size:14px;color:#4a5568;margin-top:4px;">Group: ${selectedGroup}</div>
  `;

  // لوجو يمين
  const logoRight = document.createElement('img');
  logoRight.src = college.logoRight;
  logoRight.style.height = '60px';
  logoRight.style.width = 'auto';
  logoRight.crossOrigin = 'anonymous';
  ...
  footer.innerHTML = `
    <div style="font-size:20px;color:#4a5568;margin-bottom:16px;">Approved by:</div>
    <div style="font-size:20px;font-weight:bold;color:#2d3748;border-top:1px solid #718096;display:inline-block;padding-top:4px;min-width:180px;">
      ${college.signature}
    </div>
  `;
  ...
  setTimeout(() => {
    html2canvas(tempDiv, {
      ...
      let pdfWidth = pageWidth - 8; // هامش 4مم من كل جانب
      let pdfHeight = pdfWidth * ratio;

      // ضبط الأبعاد إذا كانت أكبر من الصفحة
      if (pdfHeight > pageHeight - 8) {
        pdfHeight = pageHeight - 8;
        pdfWidth = pdfHeight / ratio;
      }

      // إضافة الصورة إلى PDF (مضبوطة من أعلى الصفحة)
  ...
  const header = `
    <div style="text-align:center;padding:20px;border-bottom:2px solid #e2e8f0;">
      <h2 style="color:#2d3748;margin:0;font-family:'Arial', sans-serif;">Academic Year: ${getYearLabel(selectedYear)}</h2>
  ...
  try {
    ...
    for (const year of Object.keys(organizedData)) {
      for (const group of Object.keys(organizedData[year])) {
        ...
        pdf.text(`Academic Year: ${getYearLabel(year)}`, pageWidth / 2, 10, { align: 'center' });
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/api/schedule/TimeTable`,
          {
            headers: {
