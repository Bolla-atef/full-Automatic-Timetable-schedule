...
const TableRatingManagement = () => {
    ...
    const fetchTables = async () => {
      try {
        ...
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/api/RatingTimeTable`,
          {
            headers: {
  ...
  const handleSwitchTable = async (tableName) => {
    try {
      const token = localStorage.getItem("userToken");
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/api/RatingTimeTable/${tableName}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleDeleteTable = async () => {
    ...
    try {
      const token = localStorage.getItem("userToken");
      await axios.delete(
        `${import.meta.env.VITE_API_URL}/api/RatingTimeTable/${selectedTable}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
