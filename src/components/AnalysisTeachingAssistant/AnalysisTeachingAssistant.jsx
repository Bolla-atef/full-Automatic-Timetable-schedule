    ...
    const fetchTeachingAssistantData = async () => {
      try {
        ...
        const [assistantsResponse, unassignedResponse] = await Promise.all([
          axios.get(
            `${import.meta.env.VITE_API_URL}/api/AnalysisAndStatisticals/TeachingAssistant`,
            {
              headers: {
          ...
          axios.get(
            `${import.meta.env.VITE_API_URL}/api/AnalysisAndStatisticals/TeachingAssistantNotMapped`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
                "Content-Type": "application/json",
