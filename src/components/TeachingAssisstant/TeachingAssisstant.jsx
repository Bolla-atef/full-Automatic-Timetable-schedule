...
const ProfessorCoursesManager = ({
  ...
  const { data: teachingAssistantCourses = [], isLoading: assignedLoading } =
    useQuery({
      ...
      queryFn: async () => {
        const token = localStorage.getItem("userToken");
        const { data } = await axios.get(
          `${import.meta.env.VITE_API_URL}/api/CourseTeachingAssistant`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
  ...
  const assignMutation = useMutation({
    mutationFn: (courseId) =>
      axios.post(
        `${import.meta.env.VITE_API_URL}/api/CourseTeachingAssistant/${professorId}/${courseId}`,
        {},
        {
          headers: {
...
const GetTeachingAssistant = () => {
  ...
  const mutation = useMutation({
    mutationFn: async (payload) => {
      ...
      const url = isEdit
        ? `${import.meta.env.VITE_API_URL}/api/TeachingAssistants/${payload.id}`
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id) => {
      const token = localStorage.getItem("userToken");
      await axios.delete(
        `${import.meta.env.VITE_API_URL}/api/TeachingAssistants/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  const handleSubmit = (e) => {
    ...
    if (!professor.name.trim() || professor.availability.length === 0) {
      toast.error("يرجى إدخال اسم مساعد التدريس والأيام المتاحة");
      return;
    }
    ...
    const payload = {
      id: isEdit ? parseInt(professor.id, 10) : 0,
                          ...
                          filteredProfessors.map((professor) => (
                            <Fragment key={professor.id}>
                              <tr key={professor.id} className="hover:bg-black">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-white">
                                  {courseProfessors.filter(cp => cp.idTeachingAssistant === professor.id).length}
                                </td>
